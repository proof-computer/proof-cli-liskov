...
COMMANDS
  ...
  application source-binding show APP_REF
  application source-binding revoke APP_REF --expected-revision N --reason TEXT --yes
  ...
  application secrets APP_REF [--json]
  application vars list APP_REF [--json]
  application vars set APP_REF NAME VALUE [--yes] [--json]
  application vars unset APP_REF NAME [--yes] [--json]
  ...
  admin retirement adjudicate-lineage UID --operation OP --expect-organization ORG --expect-application APP --expect-application-uid UID --expect-operation-kind KIND --expect-operation-status STATUS --expect-deployment DEP --expect-job JOB --expect-reserve RESERVE --expect-reserve-status STATUS --expect-billing-transaction TX --expect-billing-status STATUS --actor-id ID --evidence-ref REF --evidence-sha256 SHA256 --reason TEXT [--confirm --fingerprint SHA]
  admin retirement historical-closeout UID --manifest-ref REF --expect-retirement RET --expect-assessment-digest SHA256 --actor-id ID --reason TEXT [--confirm --fingerprint SHA]
