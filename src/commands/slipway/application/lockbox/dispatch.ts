...
export default class SlipwayApplicationLockboxDispatch extends Command {
  ...
  static description = "Dispatch the Slipway Lockbox upload workflow.";
  static examples = [
    "<%= config.bin %> slipway application lockbox dispatch proof-docs --yes",
    "<%= config.bin %> slipway application lockbox dispatch proof-docs --ref release --json --yes"
  ];
